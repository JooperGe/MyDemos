package com.jooper.mydemos.view;
...
public class RunningTextView extends TextView {
	...
	private double nowNumber = 0;// 显示的时间
	...
	private void init() {
		...
		formater = new DecimalFormat("00");// 最多两位小数，而且不够两位整数用0占位。可以通过setFormat再次设置
		handler = new Handler() {
			@Override
	 ...
	 * @param endNumber
	 */
	public void playNumber(double startNumber, double endNumber) {
		if (endNumber == 0) {
			RunningTextView.this.setText("0");
			return;
		}
		content = endNumber;// 设置最后要显示的数字
		nowNumber = startNumber;// 默认都是从0开始动画
